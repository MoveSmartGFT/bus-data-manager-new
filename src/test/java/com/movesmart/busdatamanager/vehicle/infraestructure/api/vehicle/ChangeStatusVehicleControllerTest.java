...
public class ChangeStatusVehicleControllerTest {
    ...
    void testChangeStatus() throws Exception {
        ...
        ChangeStatusVehicleRequest changeStatusRequest =
                new ChangeStatusVehicleRequest(vehicle.getPlateNumber(), newStatus);
    ...
    @Test
    @DisplayName(
            "GIVEN a vehicle change status request is received WHEN the vehicle does not exist THEN returns status 404")
    void testChangeStatusVehicleDoesNotExist() throws Exception {
        ...
        mockMvc.perform(patch(
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(
                                objectMapper.writeValueAsString(new ChangeStatusVehicleRequest("Vehicle1", newStatus))))
                .andExpect(status().isNotFound());
    }

    @Test
    @DisplayName(
            "GIVEN a vehicle change status request is received WHEN the vehicle already has that status THEN returns status 409")
