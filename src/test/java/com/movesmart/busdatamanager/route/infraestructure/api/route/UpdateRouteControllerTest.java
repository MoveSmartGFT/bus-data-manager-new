...
public class UpdateRouteControllerTest {
    ...
    @Test
    @DisplayName(
            "GIVEN a route update request is received WHEN the route exists THEN returns route object updated and status 200")
    void testUpdate() throws Exception {
        ...
        when(routeManagementUseCase.update(any())).thenReturn(route);

        mockMvc.perform(put(RouteController.ROUTE_PATH + RouteController.ROUTE_ID_PATH, route.getId())
    ...
    @Test
    @DisplayName(
            "GIVEN a route update request is received WHEN the route or a stop does not exist THEN returns status 404")
    void testUpdateRouteDoesNotExist() throws Exception {
        UpdateRouteRequest newRoute = Instancio.create(RouteInstancioModels.UPDATE_ROUTE_REQUEST_MODEL);

        when(routeManagementUseCase.update(any())).thenThrow(new EntityNotFoundException("Route", route.getId()));
