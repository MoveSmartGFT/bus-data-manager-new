...
public class GetAllStopsControllerTest {
    ...
    void testGetStop() throws Exception {
        ...
        when(stopManagementUseCase.getAll()).thenReturn(stopList);

        mockMvc.perform(get(StopController.STOP_PATH))
                .andExpect(status().isOk())
                .andExpect(json().when(Option.TREATING_NULL_AS_ABSENT)
                        .isEqualTo(objectMapper.writeValueAsString(stopList)));
    }

    @Test
    @DisplayName(
            "GIVEN all stop retrieval request is received WHEN there are no stops THEN returns empty stop list and status 200")
    void testGetStopDoesNotExist() throws Exception {
        when(stopManagementUseCase.getAll()).thenReturn(List.of());
