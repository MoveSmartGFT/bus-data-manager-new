...
public record StopResponse(@NotBlank String id, @NotBlank String name, @NotNull @Valid Coordinates location) {
    public static StopResponse fromStop(Stop stop) {
        return new StopResponse(stop.getId(), stop.getName(), stop.getLocation());
    }
}
